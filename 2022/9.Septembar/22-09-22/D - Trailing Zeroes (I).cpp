...
void prime_finding(ll n)
{
    ll sz = prime.size();
    if(n > prime[sz-1]*prime[sz-1])
    {
        for(ll i = prime[sz-1]+1 ; i*i <= n; i++)
        {
            //  cout << " number " << i ;
            if(is_prime(i))
            {
                prime.pb(i);
                // cout << "Yes it its prime";
            }
            //cout << endl;
        }
    }
