...
ll req_vote(ll x, ll mx, bool same)
{
    if(x == mx && !same)
        return 0;
   // cout << x << " max " << mx << endl;
...
void solve()
{
  ...
  if((mx == a && a == b) || (mx == b && b == c) || (mx == a && a == c))
    same = 1;

  ll reqA = req_vote(a,mx,same)  , reqB = req_vote(b,mx,same), reqC = req_vote(c,mx,same);


  cout <<reqA << " " << reqB << " " << reqC << endl;
}
int main()
{
  int test = 1;
